...
public static class OutroCutsceneSceneBuilder
{
    ...
    public static void BuildScene()
    {
        ...
        const float topBarHeight = 0f;       // sem letterbox no topo: card maior
        const float bottomBarHeight = 150f;  // barra de texto enxuta

        // Imagem da noticia: encaixada ENTRE as barras, centralizada e sem distorcer.
        ...
        // Ordem de apresentacao -> sprite (mapeado por conteudo):
        // Arapowa(02), Munduruku(03), Guarani Kaiowa(04), Wajapi(05), Apyawa(01)
        var scenes = new[]
        {
    ...
    private static Sprite LoadSprite(string name)
    {
        ...
        if (s == null) Debug.LogWarning($"[OutroCutsceneSceneBuilder] sprite nao encontrado: {Dir}/{name}.png");
        return s;
    }
    ...
    private static void SetField(object target, string fieldName, object value)
    {
        ...
        if (field == null)
        {
            Debug.LogError($"[OutroCutsceneSceneBuilder] field nao encontrado: {target.GetType().Name}.{fieldName}");
            return;
        }
